...
class LEFT_CAM_POS:
    x = 0.5 + CAM_MODULE_W / 2 # (-0.20 if LONG_TABLE else 0.40) +
    y = 0.56#0.56 if LONG_TABLE else 0.46
    height = 0.30
...
class FRONT_CAM_POS:
    x = 0# -0.80 if LONG_TABLE else 0.10
    y = 0
...
def make_camera_rig(pos, *_):
    ...
    right_anchor = m.Vector3(right_pos[0], -0.1, 0.1)
    ...
    class rig:
        ...
        front_camera = make_camera(
            "front",
            pos=front_pos + pos,
            lookat=m.Vector3(0.5, 0, 0.1) + pos, # -0.5 if LONG_TABLE else 0.5
            fovy=FOV,
        )
        ...
        def wrist_camera(name="wrist", pos="0.08 0.05 0.025", quat = "0.0442607 -0.3361931 0.9066459 0.2510147"):
