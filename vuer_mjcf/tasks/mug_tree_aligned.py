...
def register(strict=True):
    ...
    add_env(
        env_id="MugTreeAligned-fixed-v1",
        entrypoint=make_env,
        kwargs=dict(
            task=Fixed,
    ...
    add_env(
        env_id="MugTreeAligned-mug_rand-v1",
        entrypoint=make_env,
        kwargs=dict(
            task=MugRandom,
    ...
    add_env(
        env_id="MugTreeAligned-ur-mug_rand-v1",
        entrypoint=make_env,
        kwargs=dict(
            task=MugRandom,
            # the top is used for debug, will need to figure out how to render this in the future.
    ...
    add_env(
        env_id="MugTreeAligned-fixed-domain_rand-v1",
        entrypoint=make_env,
        kwargs=dict(
            task=Fixed,
    ...
    add_env(
        env_id="MugTreeAligned-mug_rand-domain_rand-v1",
        entrypoint=make_env,
        kwargs=dict(
            task=MugRandom,
            camera_names=["right", "left", "wrist",],
    ...
    add_env(
        env_id="MugTreeAligned-mug_rand-domain_rand-eval-v1",
        entrypoint=make_env,
        kwargs=dict(
            task=MugRandom,
            camera_names=[
    ...
    add_env(
        env_id="MugTreeAligned-fixed-gsplat-v1",
        entrypoint=make_env,
        kwargs=dict(
            task=Fixed,
            camera_names=["right", "left", "wrist", ],
    ...
    add_env(
        env_id="MugTreeAligned-mug_rand-gsplat-v1",
        entrypoint=make_env,
        kwargs=dict(
            task=MugRandom,
            camera_names=["right", "left", "wrist",],
    ...
    add_env(
        env_id="MugTreeAligned-ur-mug_rand-gsplat-v1",
        entrypoint=make_env,
        kwargs=dict(
            task=MugRandom,
            camera_names=["right", "left", "wrist",],
    ...
    add_env(
        env_id="MugTreeAligned-fixed-gsplat-v2",
        entrypoint=make_env,
        kwargs=dict(
            task=Fixed,
            camera_names=["right", "left", "wrist", ],
    ...
    add_env(
        env_id="MugTreeAligned-mug_rand-gsplat-v2",
        entrypoint=make_env,
        kwargs=dict(
            task=MugRandom,
            camera_names=["stereo_right", "stereo_left", "wrist",],
            xml_renderer=make_schema,
            workdir=Path(__file__).parent,
            mode="gsplat",
            gsplat_path = f"{os.environ['LUCIDSIM_EVAL_DATASETS']}/splats/cic_11th_kitchen",
    ...
    add_env(
        env_id="MugTreeAligned-mug_rand-gsplat-v3",
        entrypoint=make_env,
        kwargs=dict(
            task=MugRandom,
            camera_names=["stereo_left", "stereo_right", "wrist",],
            xml_renderer=make_schema,
            workdir=Path(__file__).parent,
            mode="gsplat",
    ...
    add_env(
        env_id="MugTreeAligned-ur-mug_rand-gsplat-v2",
        entrypoint=make_env,
        kwargs=dict(
            task=MugRandom,
            camera_names=["right", "left", "wrist",],
    ...
    add_env(
        env_id="MugTreeAligned-fixed-lucid-v1",
        entrypoint=make_env,
        kwargs=dict(
            task=Fixed,
            camera_names=["right", "left", "wrist"],
    ...
    add_env(
        env_id="MugTreeAligned-mug_rand-lucid-v1",
        entrypoint=make_env,
        kwargs=dict(
            task=MugRandom,
