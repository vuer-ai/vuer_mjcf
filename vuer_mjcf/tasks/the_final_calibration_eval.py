...
def register(strict=True):
    ...
    add_env(
        env_id="TheFinalCalibrationEval-fixed-v1",
        entrypoint=make_env,
        kwargs=dict(
            xml_path="the_final_calibration_eval.mjcf.xml",
    ...
    add_env(
        env_id="TheFinalCalibrationEval-single_random-v1",
        entrypoint=make_env,
        kwargs=dict(
            xml_path="the_final_calibration_eval.mjcf.xml",
            ...
            workdir=Path(__file__).parent,
            camera_names=["left", "right", "wrist"],
            keyframe_file="the_final_calibration_eval.frame.yaml",
        ),
        strict=strict,
    )
